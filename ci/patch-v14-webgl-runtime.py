...
def webgl_fast_path(w: str, h: str, pixels: str) -> str:
    return f'''var __otW={w},__otH={h},__otPixels={pixels};Module.SDL2||(Module.SDL2={{}});var __otSDL=Module.SDL2,__otCanvas=Module.canvas;if(__otSDL.__openttdPresenterCanvas!==__otCanvas){{__otSDL.__openttdPresenterCanvas=__otCanvas;__otSDL.__openttdWebGLPresenter=null;try{{var __otAttrs={{alpha:false,antialias:false,depth:false,stencil:false,preserveDrawingBuffer:false,premultipliedAlpha:false}},__otGl=__otCanvas.getContext("webgl2",__otAttrs),__otIs2=!!__otGl;if(!__otGl)__otGl=__otCanvas.getContext("webgl",__otAttrs)||__otCanvas.getContext("experimental-webgl",__otAttrs);if(__otGl){{var __otCompile=function(type,source){{var shader=__otGl.createShader(type);__otGl.shaderSource(shader,source);__otGl.compileShader(shader);if(!__otGl.getShaderParameter(shader,__otGl.COMPILE_STATUS))throw new Error(__otGl.getShaderInfoLog(shader)||"shader compile failed");return shader}},__otVsSource=__otIs2?"#version 300 es\\nin vec2 a;out vec2 v;void main(){{gl_Position=vec4(a,0.0,1.0);v=vec2((a.x+1.0)*0.5,(1.0-a.y)*0.5);}}":"attribute vec2 a;varying vec2 v;void main(){{gl_Position=vec4(a,0.0,1.0);v=vec2((a.x+1.0)*0.5,(1.0-a.y)*0.5);}}",__otFsSource=__otIs2?"#version 300 es\\nprecision mediump float;in vec2 v;uniform sampler2D t;out vec4 outColor;void main(){{vec3 c=texture(t,v).rgb;outColor=vec4(c,1.0);}}":"precision mediump float;varying vec2 v;uniform sampler2D t;void main(){{vec3 c=texture2D(t,v).rgb;gl_FragColor=vec4(c,1.0);}}",__otVs=__otCompile(__otGl.VERTEX_SHADER,__otVsSource),__otFs=__otCompile(__otGl.FRAGMENT_SHADER,__otFsSource),__otProgram=__otGl.createProgram();__otGl.attachShader(__otProgram,__otVs);__otGl.attachShader(__otProgram,__otFs);__otGl.linkProgram(__otProgram);if(!__otGl.getProgramParameter(__otProgram,__otGl.LINK_STATUS))throw new Error(__otGl.getProgramInfoLog(__otProgram)||"program link failed");__otGl.useProgram(__otProgram);var __otBuffer=__otGl.createBuffer();__otGl.bindBuffer(__otGl.ARRAY_BUFFER,__otBuffer);__otGl.bufferData(__otGl.ARRAY_BUFFER,new Float32Array([-1,-1,1,-1,-1,1,1,1]),__otGl.STATIC_DRAW);var __otLoc=__otGl.getAttribLocation(__otProgram,"a");__otGl.enableVertexAttribArray(__otLoc);__otGl.vertexAttribPointer(__otLoc,2,__otGl.FLOAT,false,0,0);var __otTexture=__otGl.createTexture();__otGl.activeTexture(__otGl.TEXTURE0);__otGl.bindTexture(__otGl.TEXTURE_2D,__otTexture);__otGl.texParameteri(__otGl.TEXTURE_2D,__otGl.TEXTURE_MIN_FILTER,__otGl.NEAREST);__otGl.texParameteri(__otGl.TEXTURE_2D,__otGl.TEXTURE_MAG_FILTER,__otGl.NEAREST);__otGl.texParameteri(__otGl.TEXTURE_2D,__otGl.TEXTURE_WRAP_S,__otGl.CLAMP_TO_EDGE);__otGl.texParameteri(__otGl.TEXTURE_2D,__otGl.TEXTURE_WRAP_T,__otGl.CLAMP_TO_EDGE);__otGl.uniform1i(__otGl.getUniformLocation(__otProgram,"t"),0);__otGl.pixelStorei(__otGl.UNPACK_ALIGNMENT,4);__otSDL.__openttdWebGLPresenter={{gl:__otGl,program:__otProgram,buffer:__otBuffer,texture:__otTexture,isWebGL2:__otIs2,w:0,h:0,cw:0,ch:0}};__otSDL.__openttdWebGL2ZeroCopy=__otIs2;}}}}catch(e){{console.warn("[OpenTTD perf] WebGL framebuffer presenter unavailable; using Canvas2D fallback.",e)}}}}var __otPresenter=__otSDL.__openttdWebGLPresenter;if(__otPresenter){{var __otGl=__otPresenter.gl;if(__otPresenter.cw!==__otCanvas.width||__otPresenter.ch!==__otCanvas.height){{__otGl.viewport(0,0,__otCanvas.width,__otCanvas.height);__otPresenter.cw=__otCanvas.width;__otPresenter.ch=__otCanvas.height}}__otGl.useProgram(__otPresenter.program);__otGl.bindBuffer(__otGl.ARRAY_BUFFER,__otPresenter.buffer);__otGl.bindTexture(__otGl.TEXTURE_2D,__otPresenter.texture);if(__otPresenter.w!==__otW||__otPresenter.h!==__otH){{if(__otPresenter.isWebGL2){{__otGl.texImage2D(__otGl.TEXTURE_2D,0,__otGl.RGBA,__otW,__otH,0,__otGl.RGBA,__otGl.UNSIGNED_BYTE,null);__otGl.texSubImage2D(__otGl.TEXTURE_2D,0,0,0,__otW,__otH,__otGl.RGBA,__otGl.UNSIGNED_BYTE,HEAPU8,__otPixels)}}else{{var __otSrc=HEAPU8.subarray(__otPixels,__otPixels+__otW*__otH*4);__otGl.texImage2D(__otGl.TEXTURE_2D,0,__otGl.RGBA,__otW,__otH,0,__otGl.RGBA,__otGl.UNSIGNED_BYTE,__otSrc)}}__otPresenter.w=__otW;__otPresenter.h=__otH}}else if(__otPresenter.isWebGL2){{__otGl.texSubImage2D(__otGl.TEXTURE_2D,0,0,0,__otW,__otH,__otGl.RGBA,__otGl.UNSIGNED_BYTE,HEAPU8,__otPixels);__otSDL.__openttdWebGL2ZeroCopyUploads=(__otSDL.__openttdWebGL2ZeroCopyUploads||0)+1}}else{{var __otSrc=HEAPU8.subarray(__otPixels,__otPixels+__otW*__otH*4);__otGl.texSubImage2D(__otGl.TEXTURE_2D,0,0,0,__otW,__otH,__otGl.RGBA,__otGl.UNSIGNED_BYTE,__otSrc)}}__otGl.drawArrays(__otGl.TRIANGLE_STRIP,0,4);return}}'''
...
def patch_renderer(path: Path) -> None:
    ...
    print(f"WebGL2 zero-copy presenter patched in Emscripten entry {entry_id}; WebGL1/Canvas2D fallbacks preserved")
