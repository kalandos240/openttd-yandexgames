#!/usr/bin/env python3
"""V26: preserve native pixel geometry while keeping V25 WebGL performance.

V24's desktop host recovery could synthesize a canvas size larger than the actual
iframe viewport. The native SDL software surface still follows the real child
viewport, so V25's WebGL presenter could scale that smaller surface to the larger
synthetic canvas. With NEAREST filtering this is visible as coarse/chunky image
quality after DevTools/host-size transitions.

V26 makes the actual child viewport authoritative again (V23 behavior) and adds
an exact-pixel WebGL guard: framebuffer pixels are never stretched to a different
canvas size. Gameplay, AI, mobile input, NewGRF cleanup and save/ranking logic are
not changed.
"""
...
(() => {
  'use strict';
  if (window.__openttdAdaptiveViewportV26Installed) return;
  window.__openttdAdaptiveViewportV26Installed = true;
  ...
  const apply = () => {
    ...
    try {
      ...
      /* Native framebuffer dimensions and HTML canvas backing dimensions stay 1:1. */
      ...
      console.warn('[OpenTTD] Adaptive V26 viewport resize failed', error);
      return false;
    } finally {
      applying = false;
    }
  ...
  if (typeof ResizeObserver === 'function') {
    ...
    window.__openttdAdaptiveViewportResizeObserverV26 = ro;
  }
...
def patch_runtime(path: Path) -> None:
    ...
    if '__openttdWebGLPresenter' not in text:
        raise SystemExit('V26 requires the V25 WebGL presenter')

    old = '__otGl.viewport(0,0,__otCanvas.width,__otCanvas.height);__otGl.useProgram(__otPresenter.program);'
    new = ('var __otCanvasW=__otCanvas.width|0,__otCanvasH=__otCanvas.height|0;'
    ...
    if count != 1:
        raise SystemExit(f'V25 WebGL viewport anchor count={count}')
    text = text.replace(old, new, 1)
    if '__openttdPresenterMismatch' not in text or '__otExactPixels' not in text:
        raise SystemExit('V26 WebGL exact-pixel guard missing')
...
def main() -> None:
    ...
    if 'Adaptive V24 host-aware full-viewport desktop + touch layout.' not in current:
        raise SystemExit('V26 requires V24/V25 viewport baseline')

    viewport.write_text(VIEWPORT_V26, encoding='utf-8')
    patch_runtime(runtime)

    final = viewport.read_text(encoding='utf-8')
    for needle in (
        'Adaptive V26 exact child-viewport + native-pixel layout.',
        "mode: 'actual-child-viewport'",
        'module.setCanvasSize(box.width, box.height)',
        ...
        if needle not in final:
            raise SystemExit(f'Missing V26 viewport invariant: {needle}')
    for forbidden in ('window.outerWidth', 'window.outerHeight', 'recoverDesktopBox', 'fullscreen.request()', 'MAX_DESKTOP_ASPECT_RATIO', 'setInterval(apply, 250)'):
        if forbidden in final:
            raise SystemExit(f'Forbidden V26 geometry pattern survived: {forbidden}')

    print('Adaptive V26 exact native-pixel geometry applied')
