#!/usr/bin/env python3
"""V7 mobile gestures: preserve original OpenTTD construction semantics.

Gesture arbitration is based on native OpenTTD state:
- normal viewport: one finger pans, tap clicks;
- viewport with active placement tool: tap builds and one-finger drag is sent as
  the original SDL LMB down/motion/up sequence used by desktop OpenTTD;
- GUI: original click/drag sequence;
- two fingers: camera pan + pinch zoom without placing construction.
"""
...
TOUCH_BRIDGE = r'''  const installTouchBridge = () => {
    ...
    let down = null;
    let last = null;
    let gestureMode = 'none'; // map | build | ui | multi
    let dragging = false;
    ...
    const stats = window.__openttdMobileTouchStats = {
      version: 'V7-native-context-original-build-drag',
      mapPans: 0,
      twoFingerPans: 0,
      buildDrags: 0,
      uiDrags: 0,
    ...
    const finishPointer = e => {
      ...
      if (wasPrimary && down) {
        if ((gestureMode === 'build' || gestureMode === 'ui') && mouseDownSent) {
          nativeMouse(0, p);
          nativeMouse(2, p);
        } else if (!dragging && !longPressFired && pointers.size === 0) {
          stats.taps++;
    ...
    console.info('[OpenTTD mobile] V7 native-context construction gestures installed', {
      profile,
...
def main() -> None:
    ...
    print(f'V7 native-context construction gestures applied: unpacked_bytes={unpacked}')
