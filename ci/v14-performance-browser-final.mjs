...
if (!platform || !url || !output || !['baseline', 'optimized'].includes(profile) ||
  ...
  throw new Error('usage: v14-performance-browser-final.mjs <yandex|playgama> <url> <output.json> [baseline|optimized] [map_log2=12] [ai_sample_ms=30000] [auto|during|after]');
}
...
async function armFrameProbe() {
  /* Two real animation frames are required after reset. Without this, newgame
   * can block the main thread before p.lastFrame has ever been populated; the
   * first post-generation rAF would then have no pre-generation timestamp and
   * the benchmark would falsely wait until its timeout. */
  await page.evaluate(() => new Promise(resolve => {
    requestAnimationFrame(() => requestAnimationFrame(resolve));
  }));
  const armed = await page.evaluate(() => {
    const p = window.__otPerfProbe;
    return { frames: p?.frames || 0, lastFrame: p?.lastFrame || 0, startedAt: p?.startedAt || 0 };
  });
  if (armed.frames < 2 || !(armed.lastFrame > 0) || !(armed.startedAt > 0)) {
    throw new Error(`Frame probe did not arm before newgame: ${JSON.stringify(armed)}`);
  }
  result.frameProbeArmed = true;
  return armed;
}
...
try {
  ...
  result.generation.probeBeforeNewgame = await armFrameProbe();
  ...
  if (!result.frameProbeArmed) throw new Error('Generation frame probe was not armed');
