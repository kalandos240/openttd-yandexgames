...
This script deliberately avoids the legacy BaNaNaS TCP metadata protocol.
...
USER_AGENT = "openttd-yandexgames-playgama-v6-packager/1.0"
...
def extract_verified_grf(archive: Path, output_dir: Path, expected_md5: str) -> Path:
    code = extract_main(["extract_newgrf_archive.py", str(archive), str(output_dir), expected_md5])
    if code != 0:
        raise RuntimeError(f"Could not extract verified GRF from {archive}")
    candidates = [p for p in output_dir.rglob("*.grf") if file_md5(p).lower().startswith(expected_md5.lower())]
    if len(candidates) != 1:
        raise RuntimeError(f"Expected one MD5-matching GRF in {archive}, got {candidates}")
    return candidates[0]
...
def copy_notice(search_root: Path, target: Path) -> bool:
    ...
    if not candidates:
        return False
    candidates.sort(key=lambda p: (0 if p.name.lower().startswith(("license", "copying")) else 1, len(p.parts), p.name.lower()))
    target.parent.mkdir(parents=True, exist_ok=True)
...
def prepare_prebuilt(work: Path, downloads: Path, licenses: Path) -> None:
    road_zip = download(ROAD_HOG_URL, downloads / "road-hog-1.4.1.zip")
    road_dir = downloads / "road-hog-extracted"
    road = extract_verified_grf(road_zip, road_dir, "04acd20a006e345283d6464678affA8B")
    shutil.copyfile(road, work / "road-hog-1.4.1.grf")
    ...
    gist = extract_verified_grf(gist_zip, gist_dir, "e314aaa0")
    shutil.copyfile(gist, work / "gist-0.21.10.grf")
    copy_notice(gist_dir, licenses / "gist-release-notice.txt")

    settings = download(OGFX2_SETTINGS_URL, work / "ogfx2-settings-0.7.grf")
    settings_md5 = file_md5(settings)
    if not settings_md5.lower().startswith("0d04bc9a"):
        raise RuntimeError(f"OpenGFX2 Settings MD5 changed: {settings_md5}")
...
def build_manifest(work: Path, addon_dir: Path) -> dict:
    specs = [
        {
...
def write_notices(work: Path, licenses: Path) -> None:
    ...
    notice.write_text(
        ...
        "- Road Hog 1.4.1 — GPL-2.0 release — official openttdcoop bundle.\n"
...
def main() -> int:
    ...
    prepare_prebuilt(work, downloads, licenses)
    manifest = build_manifest(work, addon_dir)
    (work / "OPENTTD-BUNDLED-ADDONS.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    write_notices(work, licenses)
