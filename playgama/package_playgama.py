...
def main():
    ...
    shutil.copy2(a.adapter,dist/'playgama-yandex-compat.js'); shutil.copy2(a.config,dist/'playgama-bridge-config.json'); (dist/'PLAYGAMA-INTEGRATION.txt').write_text(NOTICE,encoding='utf-8')
    bad=[]; total=0
    for p in dist.rglob('*'):
