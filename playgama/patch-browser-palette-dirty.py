...
# Do not force a full framebuffer invalidation before the 32bpp animation
# blitter runs. VideoDriver_SDL_Default::Paint() processes local_palette before
# it constructs the SDL update rectangle, so PaletteAnimate() can provide the
# exact browser dirty regions itself.
...
\t/* The framebuffer pixels are already updated exactly as in stock OpenTTD.
\t * Invalidate only coarse regions which actually contain animated pixels. */
